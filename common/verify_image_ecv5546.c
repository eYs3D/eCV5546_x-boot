...
int ecv5546_ecies_curve25519_decrypt(const u8 *receiver_Kpriv, const u8 *ephemeral_Kpub,
...
static int ecv5546_load_otp_Sb_pub_key(u8 in_pub[32])
{
	int ret = 0;
	...
	//#include "../../../build/tools/secure_ecv5546/secure/otp_Sb_keys/ed_pub_0.inc"
...
static int ecv5546_load_otp_Device_priv_key(u8 in_priv[32])
{
	int ret = 0;
	...
	//#include "../../../build/tools/secure_ecv5546/secure/otp_Device_keys/ed_pub_0.inc"
...
static struct sb_info *ecv5546_get_sb_info(const struct image_header  *hdr)
{
...
static int ecv5546_verify_image_signature(const struct image_header *hdr, struct sb_info *xsb)
{
	__ALIGN4
	u8 h_val[64], sig[64];
	__ALIGN4
	...
	/* Load public key */
	if (ecv5546_load_otp_Sb_pub_key(in_pub)) {
...
static int ecv5546_decrypt_image(const struct image_header *hdr, struct sb_info *xsb, u8 *cipher_bin)
{
	__ALIGN4
	u8 dev_Kpriv[32];
	__ALIGN4
	...
	if (ecv5546_load_otp_Device_priv_key(dev_Kpriv)) {
	...
	ret = ecv5546_ecies_curve25519_decrypt(dev_Kpriv, (const u8 *)xsb->eph_Kpub,
...
int ecv5546_image_verify_decrypt(const struct image_header  *hdr)
{
	int ret = 0;
	int mmu = 0;
	struct sb_info *xsb;
	...
	if (ecv5546_verify_image_signature(hdr, xsb)) {
		CSTAMP(0x23490009);
	...
	/* Decrypt uboot/fip */
	if (ecv5546_decrypt_image(hdr, xsb, (u8 *)hdr + sizeof(struct image_header))) {
		CSTAMP(0x2349000c);
